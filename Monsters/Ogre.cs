namespace DungeonExplorer.Monsters
{
    ...
    public class Ogre : Monster
    {
        ...
        /// <summary>
        /// Initialises a new instance of the <see cref="Ogre"/> class.
        /// </summary>
        public Ogre()
        ...
        public override void Attack(IDamageable target)
        {
            ...
            if (_random.Next(100) < 25) // Roll for a special attack
            {
                Console.ForegroundColor = ConsoleColor.DarkRed;
                Console.WriteLine($"{Name} roars and brings its club down HARD on {targetName}!");
                Console.ForegroundColor = ConsoleColor.White;
                int powerDamage = _random.Next(Damage, (int)(Damage * 1.6) + 1); // Calculate damage
                powerDamage = Math.Max(0, powerDamage); // Ensure damage is not negative
                target.TakeDamage(powerDamage); // Apply damage to the target
            }
            else // otherwise, use a normal attack
            {
